...
export const PropertyList = () => {
  const { data, isLoading, refetch } = useProperties({ limit: 100 })
  ...
  const handleDelete = (id: string, title: string) => {
    if (
      ...
      deleteProperty.mutate(id, {
        onSuccess: () => {
          refetch()
          dispatch(showToast({ message: 'Property deleted successfully', type: 'success' }))
        },
        onError: () => dispatch(showToast({ message: 'Failed to delete property', type: 'error' })),
      })
    }
  }
  ...
  const columns = [
    ...
    { key: 'area', header: 'Area', sortable: true, className: 'hidden md:table-cell' },
    {
      key: 'price',
      header: 'Price',
      sortable: true,
  ...
  return (
    <>
      <Seo title="All Properties" description="Manage all property listings" noindex nofollow />
      ...
      >
        ...
        >
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Manage every property listing on the platform.
            </p>
          </div>
        ...
        >
          <DataTable
            ...
            isLoading={isLoading}
            emptyMessage="No properties found. Add your first listing to get started."
            striped
            stickyHeader
          />
        </motion.div>
